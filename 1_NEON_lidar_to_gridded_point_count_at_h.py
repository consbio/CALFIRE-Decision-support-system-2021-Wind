...
version = "v4_z10n"  # Set ENV snap raster to CFO, trim edges of fishnet where there is no data.
...
# Note: The output coordinate system will match the coordinate system of the extent_fc below
...
output_fc = "_".join([r"G:\CALFIRE_Decision_support_system_2021_mike_gough\Tasks\NEON\Data\Outputs\Outputs.gdb\segmented_heights_above_ground", extent_name, las_file_date, version])
...
output_proj = arcpy.Describe(snap_grid).spatialReference
...
def pre_processing():
    ...
    arcpy.sa.ExtractMultiValuesToPoints(lidar_multipoint_to_points, [[NEON_DTM, "dtm"], [NEON_CHM, "chm"]], "NONE")

    print("Calculating height of each point from ground (DTM)")
    arcpy.AddField_management(lidar_multipoint_to_points, "height_from_ground", "DOUBLE")
    with arcpy.da.UpdateCursor(lidar_multipoint_to_points, ["Z_max", "dtm", "chm", "height_from_ground"]) as uc:
        for row in uc:
            if row[1]:
...
def count_point_returns():
    ...
    (DTM). This information is stored in a Python dictionar, and added to a fishnet for the vertical slice using an
    update cursor.
    """

    print("\nCounting Point Returns\n")

    all_h_values = [i[0] for i in arcpy.da.SearchCursor(lidar_multipoint_to_points, "height_from_ground")]
    ...
    while h_end <= h_max + 1:
        ...
        input_point_layer = arcpy.MakeFeatureLayer_management(lidar_multipoint_to_points)

...
for tile_id in tile_id_list:
    ...
    def process_lidar_points():
        ...
        with arcpy.da.UpdateCursor(lidar_points, ["Z_max", "dtm", "chm", "height_from_ground"]) as uc:
            for row in uc:
                if row[1] is not None and row[2] is not None: # Note that previously this was removing points where the CHM was 0 because used "if row[2]". DTM would never be zero, so that was not affected.
